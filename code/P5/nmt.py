...
def init_params(options):
    ...
    # dense conv
    params['conv_D']= conv_norm_weight(1, len(options['dim_enc']), 1)
...
def build_model(tparams, options):
    ...
    for i in range(len(hidden_sizes)):
        ...
        if i == 0:
            ...
            h = concatenate([proj[0], projr[0][::-1]], axis=proj[0].ndim - 1)
            h_ = concatenate([h_, h[:,None,:,:]], axis = h_.ndim-3)
    ...
    h = theano.tensor.nnet.conv2d(h_, tparams['conv_D'],border_mode='half') #seqL * 1 * batch * dim.
    h = h.dimshuffle(0, 2, 3, 1)
    h = h.reshape([h.shape[0], h.shape[1], h.shape[2]]) #seqL * batch * dim.
