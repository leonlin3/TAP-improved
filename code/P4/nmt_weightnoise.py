...
def param_init_gru(options, params, prefix='gru', nin=None, dim=None):
    ...
    #LN scale and shift
    params['gain1'] = numpy.ones((dim*2,), dtype='float32')
    params['bias1'] = numpy.zeros((dim*2,), dtype='float32')
    params['gain2'] = numpy.ones((dim*2,), dtype='float32')
    params['bias2'] = numpy.zeros((dim*2,), dtype='float32')
    params['gain3'] = numpy.ones((dim,), dtype='float32')
    params['bias3'] = numpy.zeros((dim,), dtype='float32')
    params['gain4'] = numpy.ones((dim,), dtype='float32')
    params['bias4'] = numpy.zeros((dim,), dtype='float32')
...
def build_model(tparams, options):
    ...
    for i in range(len(hidden_sizes)):
        ...
        if i == 0:
            ...
            gap = hidden_sizes[i-1]
            tmp, tmpr = h[:,:,:gap], h[:,:,gap:]
            h=concatenate([proj[0]+tmp, projr[0][::-1]+tmpr], axis=proj[0].ndim-1)
